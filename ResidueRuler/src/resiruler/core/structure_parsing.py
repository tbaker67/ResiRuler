...
def check_input_structure(structure):
    ...
    if len(structure) > 1:
        raise ValueError(
            "Multiple models found in the structure. Please provide a single model."
        )
...
def extract_residues_from_structure(structure, chains=None):
    ...
    if chains is None:
        ...
        residues = [
            ...
            if res.id[0] == " " and res.get_parent().get_id() in chains
        ]
        return residues
...
def extract_residues_by_type(structure, residue_type, chains):
    ...
    if residue_type not in set(
        ...
        raise ValueError(
            "Unknown Residue Type. Please enter a valid 3 letter type (ie LYS)"
        )
    ...
    if chains is None:
        residues = [
            res
            for res in structure.get_residues()
            if res.id[0] == " " and res.get_resname() == residue_type
        ]
        return residues
        ...
        residues = [
            ...
            for res in structure.get_residues()
            if res.id[0] == " "
            and res.get_parent().get_id() in chains
...
def get_SC_from_residue(res):
    ...
    for atom in res:
        sidechain_atom_coords = [
            atom.get_coord() for atom in res if atom.get_id() not in BACKBONE_ATOMS
        ]
...
class ChainInfo:
    ...
    def detect_chain_type(self, res_list):
        ...
        for res in res_list:
            ...
            if res_type is None:
                print(
                    f"[WARNING] Unknown residue '{res.resname}' in chain {self.chain.id}"
                )
                continue
        ...
        if not res_types:
            ...
            print(
                f"[Warning] chain {self.chain.id} contains a mix of RNA/DNA/PROTEIN residues"
            )
            return "unknown"
    ...
    def valid_pairs(self, other_collection):
        ...
        for ref_id, ref_info in self.chains.items():
            for tgt_id, tgt_info in other_collection.chains.items():
                if (
                    ref_info.type != tgt_info.type
                    or ref_info.type == "unknown"
                    or tgt_info.type == "unknown"
                ):
                    continue
