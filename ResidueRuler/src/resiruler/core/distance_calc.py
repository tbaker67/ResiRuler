...
class DistanceMatrix:
    ...
    def convert_to_df(self, lower_threshold=None, upper_threshold=None):
        ...
        df = pd.DataFrame(
            {
                "ChainID_Resnum1": [
                    f"{keys[i][0]}-{keys[i][1][1]}{keys[i][1][2]}" for i in triu_i
                ],
                "ChainID_Resnum2": [
                    f"{keys[j][0]}-{keys[j][1][1]}{keys[j][1][2]}" for j in triu_j
                ],
    ...
    def convert_to_df_chunked(
        self, lower_threshold=None, upper_threshold=None, chunk_size=5000
    ):
        keys = list(self.index_map.keys())
        n = len(keys)
        ...
        for start_i in range(0, n, chunk_size):
            ...
            for i in range(start_i, end_i):
                ...
                df_chunk = pd.DataFrame(
                    {
                        "Res1": [f"{keys[i][0]}-{keys[i][1][1]}{keys[i][1][2]}"]
                        * len(j_indices),
                        "Res2": [
                            f"{keys[j][0]}-{keys[j][1][1]}{keys[j][1][2]}"
                            for j in j_indices
                        ],
                        "Distance": distances,
                    }
...
class CompareDistanceMatrix:
    def __init__(self, reference_matrix, target_matrix, res_id_mapping):
        self.shared_keys = set(reference_matrix.index_map.keys()) & set(
            target_matrix.index_map.keys()
        )
        ...
        self.ref_coords = np.array(
            [
                reference_matrix.coords[reference_matrix.index_map[key]]
                for key in sorted(self.shared_keys)
            ],
            dtype=np.float32,
        )
        self.tgt_coords = np.array(
            [
                target_matrix.coords[target_matrix.index_map[key]]
                for key in sorted(self.shared_keys)
            ],
            dtype=np.float32,
        )
    ...
    def get_coords(self, key):
        return (
            self.ref_coords[self.index_map[key]],
            self.tgt_coords[self.index_map[key]],
        )
    ...
    def convert_to_df(self, include_coords=False):
        ...
        tgt_labels = np.array(
            [
                f"{self.res_id_mapping[k][0]}-{self.res_id_mapping[k][1][1]}{self.res_id_mapping[k][1][2]}"
                for k in keys
            ]
        )
    ...
    def get_submatrix(self, chain_ids=None, residue_ranges=None):
        ...
        new_cdm.res_id_mapping = {
            k: self.res_id_mapping[k] for k in filtered_keys if k in self.res_id_mapping
        }
    ...
    def export_to_csv_streaming(self, filepath, chunk_size=5000):
        ...
        with open(filepath, "w") as f:
            f.write(
                "ChainID_Resnum1_ref,ChainID_Resnum2_ref,ChainID_Resnum1_tgt,ChainID_Resnum2_tgt,delta_distance\n"
            )
