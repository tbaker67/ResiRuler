...
def generate_chimera_key_string(palette, positions):
    return "key " + " ".join(
        f"{color}:{round(position)}" for color, position in zip(palette, positions)
    )
...
def generate_shift_scripts(
    df,
    ref_loaded_name,
    cif2_name,
    structure_name2,
    first_structure_id,
    write_ref_load=False,
    ref_cif_name=None,
):
    """
    ...
    pml.write(
        f"cmd.alter('{disp_name1}', 'b = _dist_map1.get((chain, resi), 0.0)', space={{'_dist_map1': _dist_map1}})\n"
    )
    pml.write(f"_dist_map2 = {dist_map2!r}\n")
    pml.write(
        f"cmd.alter('{disp_name2}', 'b = _dist_map2.get((chain, resi), 0.0)', space={{'_dist_map2': _dist_map2}})\n"
    )
...
def generate_multiple_displacement_scripts(
    displacement_dfs, ref_name, palette, positions
):
    ...
    chimera_coloring_palette_string = generate_chimera_coloring_palette_string(
        palette, positions
    )
    full_cxc_script.write("open full_defattr.defattr \n")
    full_cxc_script.write(f"color #{1}-{ids - 1} grey \n")
    full_cxc_script.write(
        f"color byattribute r:distance #{1}-{ids - 1} target scab palette {chimera_coloring_palette_string}\n"
    )
    ...
    full_pml_script.write(
        f"spectrum b, {pml_palette_string}, {pml_selection}, minimum={min_val}, maximum={max_val}\n"
    )

    return (
        full_def_attr.getvalue(),
        full_cxc_script.getvalue(),
        full_pml_script.getvalue(),
    )


def chimera_displacement_vectors_from_csv(
    df, output_path=None, fidelity=1, cmap=None, norm=None
):
    """
...
def save_bild_files_and_generate_chimerax_script(
    bild_output_dict, script_name="open_all_bilds.cxc"
):
    """
    Generate a ChimeraX script to open all BILD files. Returns the script content as a string.
    """
    script_lines = []
    for filename in bild_output_dict:
...
def generate_bild_string(df, cmap=None, norm=None, fidelity=1):
    ...
    for coord1, coord2, dist in zip(coords1, coords2, distances):
        ...
        x2, y2, z2 = coord2
        r, g, b = (
            [int(255 * c) for c in cmap(norm(dist))[:3]]
            if cmap and norm
            else (255, 255, 255)
        )
    ...
    for name, df in displacement_dfs.items():
        ...
        if "ChainID_Resnum1" in df.columns:
            df_grouped = df.groupby(
                df["ChainID_Resnum1"].apply(lambda x: str(x).split("-")[0].strip())
            )
            for ref_chain, group in df_grouped:
                object_name = f"arrows_{name}_chain_{ref_chain}"
                bild_output_dict[f"{name}_chain_{ref_chain}.bild"] = (
                    generate_bild_string(group, cmap, norm, fidelity=fidelity)
                )
            ...
            bild_output_dict[name + ".bild"] = generate_bild_string(
                df, cmap, norm, fidelity=fidelity
            )
...
def save_pml_files_and_generate_pymol_script(
    pml_output_dict, script_name="run_all_arrows.pml"
):
    """
    Generate a PyMOL script to run all per-chain arrow .pml files. Returns the script content as a string.
    """
    script_lines = []
    for filename in pml_output_dict:
